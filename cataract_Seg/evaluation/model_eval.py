...
# from check_mask import tip_landmark
    ...
    for image_path in image_paths:
        ...
        with torch.no_grad():
            if detect_landmark:
                ...
                # '''
                # print(output.shape)
                # a = torch.unique(output)
                # print(a)
                mask_tensor = output.squeeze()  # 移除多余的维度，例如 (B, C, H, W) -> (H, W)
                mask_numpy = mask_tensor.cpu().numpy()

                mask_numpy_arg = np.argmax(mask_numpy, axis=0) # 未过滤最小连通区域 ！！！！！！！！！！！
                class_ids = np.unique(mask_numpy_arg)
                contours_by_class = {}
                for class_id in class_ids:
                    # 提取该类别的二值 mask
                    binary_mask = (mask_numpy_arg == class_id).astype(np.uint8)

                    # 去除小连通区域
                    filtered_binary_mask = remove_small_connected_components(binary_mask, min_area=1000)

                    # 对二值掩码进行闭运算平滑边缘
                    kernel = np.ones((7, 7), np.uint8)  # 卷积核大小，适中即可
                    smoothed_mask = cv2.morphologyEx(filtered_binary_mask, cv2.MORPH_CLOSE, kernel)
                    ...
                    for contour in contours:
                        epsilon = 0.01 * cv2.arcLength(contour, True)  # 控制平滑程度，值越大平滑越强
                        approx_contour = cv2.approxPolyDP(contour, epsilon, True)
                        smoothed_contours.append(approx_contour)

                    # 存储处理后的轮廓
                    contours_by_class[class_id] = smoothed_contours
                print('contours:', contours_by_class.keys())
                kpts_tips = []
                for cls_id, contours in contours_by_class.items():
                    if int(cls_id) > 0 and int(cls_id) < 12:
                        # print('%%%%%%%%%'*20, contours_by_class[cls_id])
                        for contour in contours:
                            print(np.array(contour).shape)
                            contour = np.reshape(np.array(contour), (-1, 2)) / (640, 640)
                            # print('contours shape:',contour.shape)
                            arr = find_tip_kpt(contour)
                ...
                for kpt in kpts_tips:
                    # print(kpt[0])
                    a, b = int(kpt[0] * h), int(kpt[1] * w)
                    print(f'draw landmark is :{a} and {b}')
                    cv2.circle(overlayed_image, (int(kpt[0] * 640), int(kpt[1] * 640)), 6, (255, 255, 255), -1)
                # print('***landmarks***:', kpts_tips)
                print('***'*20)
        if False:
...
# 过滤最小连通区域
def remove_small_connected_components(binary_mask, min_area=50):
    """
    移除小于指定面积的连通区域
    Args:
        binary_mask: 二值掩码图像 (numpy.ndarray)
        min_area: 保留的连通区域的最小面积
    Returns:
    ...
    for i in range(1, num_labels):  # 跳过背景标签 (0)
        area = stats[i, cv2.CC_STAT_AREA]  # 该连通区域的面积
        if area >= min_area:
            filtered_mask[labels == i] = 1  # 保留满足面积条件的区域
...
if __name__ == '__main__':
    ...
    model_path = r"C:\Users\Charl\PycharmProjects\cataract_Seg\checkpoint_fine_tune_colab\epoch_260.pth"
